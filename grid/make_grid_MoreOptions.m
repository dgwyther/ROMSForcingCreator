...
ModelName = 'Amery Ice Shelf Ocean Model grid, 1/30 res, strong smoothing';
...
%-----------------------------------------------------------------------------
filetitle =([ModelName,' grid file']); % Make title from model name variable
...
disp('make model grid')
%make model grid (geographic and psg at geographic points)
...
SourceData = 'bedmap2icertopobathy' %rtopo'

if strcmp(SourceData,'bedmap2')
%% Bedmap2
%load('/ds/projects/iomp/obs/geom/bedmap2/bedmap2.mat')
load('bedmap2.mat')
disp(['loaded bedmap2 at ', datestr(now)])
lons = nan(6667,6667); lats = nan(6667,6667);
for i = 1:6667;
[t1 t2] = inverse_polar_stereo(x(i,:),y(i,:),0,0,1,-71);
lats(i,:) = t1';
lons(i,:) = t2';
end
bathym = -elev_bed;
idraft = -(thick-elev_surf); % ROMS format
idraft(isnan(idraft)) = 0;
Cwm = thick;
Cwm(~isnan(thick)) = 0;
Cwm(isnan(thick)) = 1;
Cwm(mask_rock) = 0;
Cim = double(mask_ice);
Cim(find(double(mask_rock) == 1)) = 0;
Cwm(find(double(Cim) == 1)) = 1;
im=Cim;
wm = Cwm;

% interp bedmap2 to model grid:
    pg.h1 = double(griddata(lons,lats,bathym,pg.lon,pg.lat)); disp(['interpolating bathy at ', datestr(now)]) % bathymetry
    pg.zice    = double(griddata(lons,lats,idraft,pg.lon,pg.lat)); disp(['interpolating ice at ', datestr(now)]) % ice draft
    pg.icemask    = double(griddata(lons,lats,im,pg.lon,pg.lat,'nearest')); disp(['interpolating ice mask at ', datestr(now)]) %ice mask
    pg.watermask = double(griddata(lons,lats,wm,pg.lon,pg.lat,'nearest')); disp(['interpolating water mask at ', datestr(now)]) %water mask


elseif strcmp(SourceData,'rtopo') | strcmp(SourceData,'bedmap2icertopobathy')
...
% interp rtopo to model grid:
    pg.h1      = double(-interp2(lons(Yrng,Xrng),lats(Yrng,Xrng),bathy(Yrng,Xrng),pg.lon,pg.lat)); % bathymetry
    pg.zice    = double(interp2(lons(Yrng,Xrng),lats(Yrng,Xrng),draft(Yrng,Xrng),pg.lon,pg.lat)); % ice draft
...
if strcmp(SourceData,'bedmap2icertopobathy')
load('bedmap2.mat','thick','elev_surf','x','y','mask_ice','mask_rock')
disp(['loaded bedmap2 at ', datestr(now)])
lons = nan(6667,6667); lats = nan(6667,6667);
for i = 1:6667;
[t1 t2] = inverse_polar_stereo(x(i,:),y(i,:),0,0,1,-71);
...
% interp bedmap2 to model grid:
%    pg.h1 = double(griddata(lons,lats,bathym,pg.lon,pg.lat)); disp(['interpolating bathy at ', datestr(now)]) % bathymetry
    pg.zicebedmap2    = double(griddata(lons,lats,idraft,pg.lon,pg.lat)); disp(['interpolating ice at ', datestr(now)]) % ice draft
    pg.icemaskbedmap2    = double(griddata(lons,lats,im,pg.lon,pg.lat,'nearest')); disp(['interpolating ice mask at ', datestr(now)]) %ice mask
%    pg.watermask = double(griddata(lons,lats,wm,pg.lon,pg.lat,'nearest')); disp(['interpolating water mask at ', datestr(now)]) %water mask

pg.icemaskNaN= pg.icemask; pg.icemaskNaN(pg.icemask==0)=NaN;
halo=5;
pg.icemaskhalo = pg.icemask; 
pg.icemaskhalo=pg.icemaskhalo.*cat(1,pg.icemask(halo+1:end,:),nan(halo,size(pg.icemask,2)));
pg.icemaskhalo=pg.icemaskhalo.*cat(1,nan(halo,size(pg.icemask,2)),pg.icemask(1:end-halo,:));
pg.icemaskhalo=pg.icemaskhalo.*cat(2,pg.icemask(:,halo+1:end),nan(size(pg.icemask,1),halo));
pg.icemaskhalo=pg.icemaskhalo.*cat(2,nan(size(pg.icemask,1),halo),pg.icemask(:,1:end-halo));
pg.icemaskhalo=pg.icemaskhalo.*cat(2,cat(1,pg.icemask(halo+1:end,halo+1:end),nan(halo,size(pg.icemask,2)-halo)),nan(size(pg.icemask,1),halo));
pg.icemaskhalo=pg.icemaskhalo.*cat(2,nan(size(pg.icemask,1),halo),cat(1,pg.icemask(halo+1:end,1:end-halo),nan(halo,size(pg.icemask,2)-halo)));
pg.icemaskhalo=pg.icemaskhalo.*cat(1,nan(halo,size(pg.icemask,2)),cat(2,pg.icemask(1:end-halo,halo+1:end),nan(size(pg.icemask,1)-halo,halo)));
pg.icemaskhalo=pg.icemaskhalo.*cat(1,nan(halo,size(pg.icemask,2)),cat(2,nan(size(pg.icemask,1)-halo,halo),pg.icemask(1:end-halo,1:end-halo)));

pg.icemaskEdge=zeros(size(pg.icemask)); pg.icemaskEdge(((pg.icemaskhalo-pg.icemask).*pg.icemask)==-1)=1;

pg.cavwatermask = ~(pg.watermask - pg.icemask); pg.cavwatermaskhalo=pg.cavwatermask;
pg.cavwatermaskhalo=pg.cavwatermaskhalo.*cat(1,pg.cavwatermask(halo+1:end,:),nan(halo,size(pg.cavwatermask,2)));
pg.cavwatermaskhalo=pg.cavwatermaskhalo.*cat(1,nan(halo,size(pg.cavwatermask,2)),pg.cavwatermask(1:end-halo,:));
pg.cavwatermaskhalo=pg.cavwatermaskhalo.*cat(2,pg.cavwatermask(:,halo+1:end),nan(size(pg.cavwatermask,1),halo));
pg.cavwatermaskhalo=pg.cavwatermaskhalo.*cat(2,nan(size(pg.cavwatermask,1),halo),pg.cavwatermask(:,1:end-halo));
pg.cavwatermaskhalo=pg.cavwatermaskhalo.*cat(2,cat(1,pg.cavwatermask(halo+1:end,halo+1:end),nan(halo,size(pg.cavwatermask,2)-halo)),nan(size(pg.cavwatermask,1),halo));
pg.cavwatermaskhalo=pg.cavwatermaskhalo.*cat(2,nan(size(pg.cavwatermask,1),halo),cat(1,pg.cavwatermask(halo+1:end,1:end-halo),nan(halo,size(pg.cavwatermask,2)-halo)));
pg.cavwatermaskhalo=pg.cavwatermaskhalo.*cat(1,nan(halo,size(pg.cavwatermask,2)),cat(2,pg.cavwatermask(1:end-halo,halo+1:end),nan(size(pg.cavwatermask,1)-halo,halo)));
pg.cavwatermaskhalo=pg.cavwatermaskhalo.*cat(1,nan(halo,size(pg.cavwatermask,2)),cat(2,nan(size(pg.cavwatermask,1)-halo,halo),pg.cavwatermask(1:end-halo,1:end-halo)));

pg.icemaskFront=-double(pg.cavwatermaskhalo-pg.cavwatermask).*pg.icemask; pg.icemaskFront(isnan(pg.icemaskFront))=0;


halo=1;
pg.icemaskglhalo = pg.icemask; 
pg.icemaskglhalo=pg.icemaskglhalo.*cat(1,pg.icemask(halo+1:end,:),nan(halo,size(pg.icemask,2)));
pg.icemaskglhalo=pg.icemaskglhalo.*cat(1,nan(halo,size(pg.icemask,2)),pg.icemask(1:end-halo,:));
pg.icemaskglhalo=pg.icemaskglhalo.*cat(2,pg.icemask(:,halo+1:end),nan(size(pg.icemask,1),halo));
pg.icemaskglhalo=pg.icemaskglhalo.*cat(2,nan(size(pg.icemask,1),halo),pg.icemask(:,1:end-halo));
pg.icemaskglhalo=pg.icemaskglhalo.*cat(2,cat(1,pg.icemask(halo+1:end,halo+1:end),nan(halo,size(pg.icemask,2)-halo)),nan(size(pg.icemask,1),halo));
pg.icemaskglhalo=pg.icemaskglhalo.*cat(2,nan(size(pg.icemask,1),halo),cat(1,pg.icemask(halo+1:end,1:end-halo),nan(halo,size(pg.icemask,2)-halo)));
pg.icemaskglhalo=pg.icemaskglhalo.*cat(1,nan(halo,size(pg.icemask,2)),cat(2,pg.icemask(1:end-halo,halo+1:end),nan(size(pg.icemask,1)-halo,halo)));
pg.icemaskglhalo=pg.icemaskglhalo.*cat(1,nan(halo,size(pg.icemask,2)),cat(2,nan(size(pg.icemask,1)-halo,halo),pg.icemask(1:end-halo,1:end-halo)));

pg.icemaskGL=zeros(size(pg.icemask)); pg.icemaskGL(((pg.icemaskglhalo-pg.icemask).*pg.icemask)==-1)=1;


%now set zice on GL-icemaskFront to -(bathy-minclippingdepth), set zice to nan in icemaskhalo-icemaskFront
pg.zicebedmap2_altered = pg.zicebedmap2.*pg.icemask; 
pg.zicebedmap2_altered(logical(pg.icemaskEdge.*~pg.icemaskFront))=NaN;

pg.zicebedmap2_altered(logical(pg.icemaskGL.*~pg.icemaskFront)) = -(pg.h1(logical(pg.icemaskGL.*~pg.icemaskFront)) - s.clipping_depths(1)); 

% now calculate new water column thickness, from pg.h1 and pg.zicebedmap2_altered, and inpaint it.

pg.watercolumnthickness_altered = (pg.h1+pg.zicebedmap2_altered).*pg.icemask;
pg.watercolumnthickness_altered = inpaint_nans(pg.watercolumnthickness_altered,1);
temporary=pg.watercolumnthickness_altered-pg.h1;
pg.zicebedmap2_altered2=pg.zicebedmap2_altered; pg.zicebedmap2_altered2(isnan(pg.zicebedmap2_altered)) = temporary(isnan(pg.zicebedmap2_altered));

pg.zice = pg.zicebedmap2_altered2;
...
comparisons=0
if comparisons
pg.icemaskNaN = pg.icemask; pg.icemaskNaN(pg.icemaskNaN==0)=NaN;
pgrtopo.icemaskNaN = pgrtopo.icemask; pgrtopo.icemaskNaN(pgrtopo.icemaskNaN==0)=NaN;

figure,flat_pcolor((pgrtopo.h1+pg.zice).*pg.icemaskNaN),colorbar,ntitle('(pgrtopo.h1+pg.zice).*pg.icemaskNaN')
figure,flat_pcolor((pgrtopo.h1+pg.zice).*pgrtopo.icemaskNaN),colorbar,ntitle('(pgrtopo.h1+pg.zice).*pgrtopo.icemaskNaN')
figure,flat_pcolor((pg.h1+pg.zice).*pg.icemaskNaN),colorbar,ntitle('(pg.h1+pg.zice).*pg.icemaskNaN')
figure,flat_pcolor((pgrtopo.h1+pgrtopo.zice).*pgrtopo.icemaskNaN),colorbar,ntitle('(pgrtopo.h1+pgrtopo.zice).*pgrtopo.icemaskNaN')

figure('pos',[100 100 1200 1400]),flat_pcolor(pg.h1-pgrtopo.h1),colorbar,cmocean('balance','pivot',0)
figure('pos',[100 100 1200 1400]),flat_pcolor(pg.zice-pgrtopo.zice),colorbar,cmocean('balance','pivot',0)
hold on,plot(pg.h1(:,450)),hold on,plot(pgrtopo.h1(:,450)),hold on,plot(-pg.h1(:,450)),hold on,plot(-pgrtopo.h1(:,450))
end

disp('make rho, u, v grids')
...
%Crop mountains
ice_draft(bathymetry < min_depth) = 0;
bathymetry(bathymetry < min_depth) = min_depth;


%% Smooth bathymetry
addpath(genpath('/ds/projects/iomp/matlab_scripts/ROMS_MATLAB'))
icedraft_orig = ice_draft; bathymetry_orig = bathymetry;
if 0 %old method
wct = bathymetry+ice_draft;
wcts = smoothgrid(wct,ones(size(s.mw)),0,5000,.1,2,4);
bathymetry = smoothgrid(bathymetry,ones(size(s.mw)),0,5000,.3,2,4);
ice_draft = -( bathymetry - wcts);
elseif 1 %new scripts
wct = bathymetry+ice_draft;
wcts = smooth_bath(wct,ones(size(s.mw)),4,.1,150);
bathymetry = smooth_bath(bathymetry,ones(size(s.mw)),4,.3,150);
ice_draft = -( bathymetry - wcts);
end


s.mwNaN = double(s.mw); s.mwNaN(s.mw==0)=NaN;
s.miNaN = double(s.mi); s.miNaN(s.mi==0)=NaN;
figure
flat_pcolor((bathymetry-bathymetry_orig).*s.mwNaN),colorbar,cmocean('balance','pivot',0)
figure,
flat_pcolor((ice_draft-icedraft_orig).*s.miNaN),colorbar,cmocean('balance','pivot',0)
...
%mindxy = min(s.delx,s.dely); %simple smallest dx
mindxy = 1./( sqrt( 1./s.delx.^2 + 1./s.dely.^2)); %gridspacing ala shchepetkin
C_g=.87 %for generalised FB stepping
CFL = C_g*mindxy./abs(sqrt(9.81.*wct)).*s.mwNaN; %C_g > dtfast*sqrt(gH)/mindxy, so dtfast < C_g/sqrt(gH)*mindxy
[MagMinCFL,IndMinCFL] = nanmin(CFL(:)); [IndMinCFLi,IndMinCFLj]=ind2sub(size(CFL),IndMinCFL);
figure, flat_pcolor(CFL),colorbar,title(['DTFAST max=',num2str(MagMinCFL),' to satisfy CFL=',num2str(C_g)]), hold on,plot(IndMinCFLj,IndMinCFLi,'kx'),
...
netcdf.putAtt(id, netcdf.getConstant('NC_GLOBAL'), 'title', filetitle);
