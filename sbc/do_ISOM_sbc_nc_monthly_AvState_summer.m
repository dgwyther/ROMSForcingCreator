...
title =([ModelName,'surface heat/salt fluxes and wind stress']); % Make title from model name variable

disp('load air sea flux file (big file so load it once and subset later)')
load([WorkingDir,ModelName,RunNo,'_air_sea_fluxes_ERAi_monthly.mat'])
load([WorkingDir,'ustress_grid_model_1992_2012_monthly.mat'])
load([WorkingDir,'vstress_grid_model_1992_2012_monthly.mat'])
...
disp('calculating average forcing state for summer (time av. of dec,jan,feb)')

shf = squeeze(nanmean(shf([1:3,12],:,:),1));
swf = squeeze(nanmean(swf([1:3,12],:,:),1));
su  = squeeze(nanmean(su([1:3,12],:,:),1));
sv  = squeeze(nanmean(sv([1:3,12],:,:),1));

disp('inpainting nans in wind fields...')

  shf = inpaint_nans(shf,1);
  swf = inpaint_nans(swf,1);
  su  = inpaint_nans(su,1);
  sv  = inpaint_nans(sv,1);

disp(['filling yearly file with average forcing state, i.e. filling average state into the ',num2str(time_big),' time records'])
...
%Takeshi_index=Takeshi_index+time;
