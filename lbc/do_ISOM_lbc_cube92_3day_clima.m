% Load ECCO data
load(['cube92_3day_clima_salt_',RunName,'.mat'])
load(['cube92_3day_clima_theta_',RunName,'.mat'])
load(['cube92_3day_clima_uvel_',RunName,'.mat'])
load(['cube92_3day_clima_vvel_',RunName,'.mat'])

theta(theta < -10) = NaN;
salt(salt < 0) = NaN;

% Make climatology from 3-dav avs
...
isfc.tmp = nan(365,size(LonInterpSurface,1),size(LonInterpSurface,2));
isfc.slt = nan(365,size(LonInterpSurface,1),size(LonInterpSurface,2));
isfc.u =   nan(365,size(LonInterpSurface,1),size(LonInterpSurface,2));
isfc.v =   nan(365,size(LonInterpSurface,1),size(LonInterpSurface,2));
isfc.dpt = nan(365,size(LonInterpSurface,1),size(LonInterpSurface,2));
sfc.prs =  nan(365,size(LonInterpSurface,1),size(LonInterpSurface,2));
...
if ~rem(ii,round(365/10))
disp([num2str(ii/365*100),'%'])
end
end
...
do_ISOM_lbc_nc_cube92(bryname,grdname,'Lateral Boundaries Salt and Temp',[0 1 1 1],Vtransform, Vstretching, Tcline, theta_s,theta_b,20,N,[365.25/365:365.25/365:365.25],[365.25],'clobber')
