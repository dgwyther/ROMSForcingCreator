...
isfc.tmp = nan(364,size(LonInterpSurface,1),size(LonInterpSurface,2));
isfc.slt = nan(364,size(LonInterpSurface,1),size(LonInterpSurface,2));
isfc.u =   nan(364,size(LonInterpSurface,1),size(LonInterpSurface,2));
isfc.v =   nan(364,size(LonInterpSurface,1),size(LonInterpSurface,2));
isfc.dpt = nan(364,size(LonInterpSurface,1),size(LonInterpSurface,2));
sfc.prs =  nan(364,size(LonInterpSurface,1),size(LonInterpSurface,2));
...
if ~rem(ii,round(364/10))
disp([num2str(ii/364*100),'%'])
end
end
...
do_ISOM_lbc_nc_cube92(bryname,grdname,'Lateral Boundaries Salt and Temp',[0 1 1 1],Vtransform, Vstretching, Tcline, theta_s,theta_b,20,N,[1:364],[364],'clobber')
